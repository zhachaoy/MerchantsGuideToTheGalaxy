...
public class Lexer {
    ...
    public static void processFile(String filePath) throws IOException {
        ...
        if (filePath == null) {
            InputStream in = Lexer.class.getClassLoader().getResourceAsStream("input");
    ...
    public static void convertTokentoValue() {
        ...
        while (it.hasNext()) {
            ...
            float integerValue = new Parser().romanToDecimal(token.getValue().toString());
    ...
    private static void decodeMissingQuery(String query) {
        ...
        float valueOfElementInDecimal = new Parser().romanToDecimal(stringBuilder.toString());
